<aside class="main-sidebar sidebar-dark-primary elevation-4">
    <!-- Brand Logo -->
    <a href="../index.php" class="brand-link" style="text-decoration: none;">
        <i class="fas fa-car mx-3"></i>
        <span class="brand-text font-weight-light">ระบบเช่ารถยนต์</span>
    </a>

    <!-- Sidebar -->
    <div class="sidebar">

        <!-- SidebarSearch Form -->

        <!-- Sidebar Menu -->
        <nav class="mt-2">
            <ul class="nav nav-pills nav-sidebar flex-column" data-widget="treeview" role="menu" data-accordion="false">
                <li class="nav-header">จัดการรถยนต์</li>
                <li class="nav-item">
                    <a href="?p=list_car" class="nav-link">
                        <i class="nav-icon fas fa-table"></i>
                        <p>
                            จัดการรถยนต์
                            <i class="fas fa-angle-left right"></i>
                        </p>
                    </a>

                </li>

                <li class="nav-item">
                    <a href="?p=rent_page" class="nav-link">
                        <i class="nav-icon fas fa-table"></i>
                        <p>
                            เช่ารถ
                            <i class="fas fa-angle-left right"></i>
                        </p>
                    </a>

                </li>

                <li class="nav-item">
                    <a href="?p=form_car" class="nav-link">
                        <i class="nav-icon fas fa-table"></i>
                        <p>
                            แบบฟอร์มกรอกข้อมูลรถ
                            <i class="fas fa-angle-left right"></i>
                        </p>
                    </a>

                </li>

                <li class="nav-item">
                    <a href="?p=list_rent" class="nav-link">
                        <i class="nav-icon fas fa-table"></i>
                        <p>
                            ตารางการเช่ารถ
                            <i class="fas fa-angle-left right"></i>
                        </p>
                    </a>

                </li>

            </ul>
        </nav>
        <!-- /.sidebar-menu form member -->
        <nav class="mt-2">
            <ul class="nav nav-pills nav-sidebar flex-column" data-widget="treeview" role="menu" data-accordion="false">
                <li class="nav-header">จัดการสมาชิก</li>
                <li class="nav-item">
                    <a href="?p=form_member" class="nav-link">
                        <i class="nav-icon fas fa-table"></i>
                        <p>
                            เพิ่มสมาชิก
                            <i class="fas fa-angle-left right"></i>
                        </p>
                    </a>

                </li>
                <!-- /.sidebar-menu insert-->
                <li class="nav-item">
                    <a href="?p=list_member" class="nav-link">
                        <i class="nav-icon fas fa-table"></i>
                        <p>
                            จัดการสมาชิก
                            <i class="fas fa-angle-left right"></i>
                        </p>
                    </a>

                </li>
            </ul>
        </nav>
    </div>
    <!-- /.sidebar -->
</aside>